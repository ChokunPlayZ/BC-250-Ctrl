...
"<label>New Wi-Fi password<input id=wpass type=password maxlength=64 placeholder='leave blank to keep'></label></div></section>"
...
"<label>Handoff delay (ms)<input id=handoff type=number min=0></label></div>"
...
"function renderButtons(){let e=$('buttons');e.innerHTML='';cfg.buttons.forEach((b,i)=>{let d=document.createElement('div');d.className='row';d.innerHTML=`<input type=number value='${b.gpio}' title='GPIO'><label><input type=checkbox ${b.active_high?'checked':''}>active high</label>${actionSelect(b.short_action)}${actionSelect(b.double_action)}${actionSelect(b.long_action)}<button class=danger>Remove</button>`;let q=d.querySelectorAll('input,select');q[0].onchange=x=>b.gpio=+x.target.value;q[1].onchange=x=>b.active_high=x.target.checked;q[2].onchange=x=>b.short_action=x.target.value;q[3].onchange=x=>b.double_action=x.target.value;q[4].onchange=x=>b.long_action=x.target.value;d.querySelector('button').onclick=()=>{cfg.buttons.splice(i,1);renderButtons()};e.appendChild(d)})}"
"function addButton(){cfg.buttons.push({enabled:true,gpio:-1,active_high:false,short_action:'toggle',double_action:'none',long_action:'force_off'});renderButtons()}"
...
"async function load(){cfg=await api('/api/v1/config');$('radio').value=cfg.radio_profile;$('hostname').value=cfg.hostname;$('ssid').value=cfg.wifi_ssid;$('advanced').checked=cfg.advanced_gpio_override;$('pson').value=cfg.pins.ps_on.gpio;$('pbtn').value=cfg.pins.power_button.gpio;$('sense').value=cfg.pins.power_sense.gpio;$('led').value=cfg.pins.status_led.gpio;$('strategy').value=cfg.timing.strategy;$('delay').value=cfg.timing.inter_output_delay_ms;$('pulse').value=cfg.timing.button_pulse_ms;$('handoff').value=cfg.timing.handoff_delay_ms;renderButtons();renderBle()}"
"async function save(){cfg.configured=true;cfg.radio_profile=$('radio').value;cfg.hostname=$('hostname').value;cfg.wifi_ssid=$('ssid').value;cfg.wifi_password=$('wpass').value;cfg.admin_password=$('admin').value;cfg.advanced_gpio_override=$('advanced').checked;cfg.pins.ps_on.gpio=+$('pson').value;cfg.pins.power_button.gpio=+$('pbtn').value;cfg.pins.power_sense.gpio=+$('sense').value;cfg.pins.status_led.gpio=+$('led').value;cfg.timing.strategy=+$('strategy').value;cfg.timing.inter_output_delay_ms=+$('delay').value;cfg.timing.button_pulse_ms=+$('pulse').value;cfg.timing.handoff_delay_ms=+$('handoff').value;try{await api('/api/v1/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(cfg)});msg('Saved. Rebooting…')}catch(e){msg(e,true)}}"
