...
"<section class=card><h2>HP Common Slot PSU</h2><div id=psustatus>PSU I2C disabled</div>"
"<p><small>Optional 3.3 V I2C monitoring. HP PSU on/off requires a separate hardware connection.</small></p><div class=grid>"
"<label class=row><input id=psuenabled type=checkbox> Enable PSU I2C</label>"
...
"async function status(){try{let s=await api('/api/v1/status');$('status').textContent=`Power: ${s.power_state} · sensed: ${s.sensed_on?'on':'off'} · Wi-Fi: ${s.wifi_ip} · Zigbee: ${s.zigbee_joined?'joined':'not joined'}`;let p=s.psu_i2c;$('psustatus').textContent=!p.enabled?'PSU I2C disabled':!p.available?'PSU I2C unavailable':`Input ${p.input_voltage_v.toFixed(1)} V, ${p.input_current_a.toFixed(2)} A · Output ${p.output_voltage_v.toFixed(2)} V, ${p.output_current_a.toFixed(2)} A · Internal ${p.internal_temperature_f.toFixed(1)} °F · Fan reading ${p.fan_speed_raw}`;$('otasection').hidden=!s.ota_enabled;}catch(e){$('status').textContent=e}}"
